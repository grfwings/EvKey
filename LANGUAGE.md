...
- Definitions inside procedures are locally scoped (shadow global definitions)
...
statement     ::= definition | action | comment
...
action        ::= "hold" (key | set | identifier) "for" duration
                | "tap" key
                | "wait" duration
                | "move" int int
                | "scroll" direction int
                | "run" identifier args?
                | identifier args? "for" duration
